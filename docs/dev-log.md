...
## 版本日志 + UI 完成度盘点（2026-05-23）

xin 让做一次盘点：现有 open issues + 各屏 / 功能完成度 + 验证状态 + README 加版本日志。结果：

### 新 issue（2026-05-22 / 2026-05-23 真机反馈）

- **#4 数据问题** —— 需要云端服务器 + 备份；本地数据怎么迁到服务器
- **#5 服务器问题** —— 数据存储问题
- **#6 时间提醒问题** —— 偶尔午餐 / 晚餐没提醒；新需求："离窗口结束 30min 时再提醒一次"；**bug**：早午餐窗口从 schedule -1.5h 就开始计时，应该从 schedule 起算
- **#7 onboarding 完即弹错过餐** —— 不应该弹；只应该在 onboarding 完后第一个完整窗口过期才触发

### UI 完成度

详 [README "UI 完成度盘点"段](../README.md)。重点：

- ✅ **做完验证**（iPhone TestFlight）：onboarding、Stage 1/2 hero、HP 心形条、NextMealCard、记录 tab、统计 tab、我的 tab、photo modal、Weight OCR、Stage 1 start/end/demote
- 🟡 **做了待修**：餐窗提醒卡（Issue #6 计时早 1.5h）、错过餐 modal（Issue #7 误弹）、Stage 2-5 demote（仅 dev panel 验过）、Stage 2→3+ 进阶（业务条件未接入）、YOLO 食物识别（仅内网 LAN 验，TestFlight 外网 fail-soft）、LLM mascot 文案（key 暴露）
- ⬜ **未做**：加餐 SnackCard（在分支未合）、云备份 / Apple Sign In / Sentry / Cloudflare Worker

### 阻塞 v1.1

Issue #6 + Issue #7 是真机已知 bug，**先修这两个再启动 v1.1 后端工作**，否则用户即使有云同步也用得难受。

### 同步动作

- README 加"版本日志"段（v0.1 → v1.1 规划）
- README 加"UI 完成度盘点（2026-05-23）"表
- `docs/product/changelog.md` 加 v1.1 规划段（含阻塞项 + 计划）
