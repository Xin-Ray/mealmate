...
## v0.4 hotfix #10：critical mascot 专属 ip 子组替换（2026-05-07）

之前 critical.png 是 sips 从 1:332 frame 顶部裁的占位（524×392，无专属设计）。xin 在 Figma 画了 1:357 专属 ip 子组（524×416，含"加油!"小牌子的 critical 专属姿势）。

### 改动

1. `app/assets/mascot/critical.png` 替换：
   - URL: figma asset b6eb5471
   - 524×416 PNG（之前 524×392）
   - md5 c7b500b5（旧 c429d06a3 不同 ✓，跟其他 4 mascot 全异 ✓）
2. `src/theme/hp.ts` HP_BANDS critical 的 `mascotAspect` 改 `524/392 → 524/416`
3. `HomeStage2.tsx` 的 hero card aspectRatio 524/461 不动，mascot width:90% + 自身 aspect → mascot height 在 hero 内不撑满，bottom anchor 贴底，上方留白给文字

### simulator 自截验证（`/tmp/mealmate-critical-v8.png`）

HP 0 / critical band：
- ✅ 新 mascot 完整显示：黑发 + 担忧脸 + 黑眼圈 + 围裙 + **手举"加油!"小牌子**（critical 专属姿势）+ 飘叶子 + 蒸气
...
## v0.4 hotfix #13：餐次拍照接 YOLO 食物识别 + 重拍按钮（2026-05-17）

### 背景

跟 #12 体重秤 OCR 同期 —— 餐次拍照走 YOLO（self-host backend `POST /detect`），结果展示在 result 页。同时补一个 #12 也想要的能力：result 页能"重拍一张"重新走识别，不重复扣加 HP。

### 改动

1. `app/src/services/foodDetection.ts`（新建）
   - 调 `${EXPO_PUBLIC_DETECT_API_BASE}/detect`，multipart `image` 字段
   - 12s 超时（GPU 推理 < 100ms，主要花在上传，弱网够用）
   - 失败抛错让调用方决定降级（photo.tsx 选择「识别失败也允许打卡」）
   - 附带 `/health` 检查端点
2. `app/.env.example`（新建）— `EXPO_PUBLIC_DETECT_API_BASE=http://<YOUR-SERVER-LAN-IP>:8000`
3. `app/app/(modal)/photo.tsx`
   - `onConfirm` 改 async：调 `detectFood(imageUri)`，结果存 `detections` / 错误存 `detectError`
   - **`confirmedOnce` 守卫**：同一次进 modal 重拍多次只 `markMealDone` + push dialogue 一次，避免反复 +5 HP / 反复推鼓励文案。识别结果每次都更新。
   - result 页展示识别 chips（`label · 87%`）；识别失败 fallback 文案"识别服务没连上，餐已打卡。"
   - 新「重拍一张」按钮：清 `imageUri` / `detections` / `detectError` → 回 `intro`；`confirmedOnce` 保留

### 设计决策

- **识别失败不阻塞打卡**：mealmate 核心价值是"按时吃饭"，YOLO 是锦上添花。后端挂了 / 网断了，用户拍了照仍应该 +HP，UI 提示一下识别没成即可。
- **`confirmedOnce` 在 modal 局部，不入 store**：重新进 modal（用户从首页再点一次）应该当作新一次打卡，store 那边 `markMealDone` 自带"今日已 done 不重复"的 dedup 兜底。

### 验收 TODO

- ⏳ 真机端到端：服务器在 LAN 上，手机连同段 wifi，从拍照到 chips 显示
- ⏳ 离线模式：杀掉后端进程，验证"识别服务没连上"那条 fallback 文案确实出来，且 HP 仍 +5

## v1.0.1 hotfix：餐窗起点 + 窗末 30min 二次提醒 + onboarding 漏餐守卫（2026-05-23）

来源：2026-05-22 xin 真机反馈，Issue #6 + #7。分支 `fix/issues-6-7-meal-timing`，commit `533fdcf`。

### 改动

1. `src/store/selectors/reminder.ts` `selectActiveReminderSlot`
   - 窗口起点 `scheduleTime - 90min` → `scheduleTime`
   - 旧 `[schedule-90min, schedule+90min]` 是 3h 窗（提前 1.5h 显示提醒）；新 `[schedule, schedule+90min]` 1.5h 窗
2. `src/services/notifications.ts` `scheduleMealReminders`
   - 每餐加一条 `schedule +60min`（= 窗末前 30min）的 DAILY 通知，identifier `mealmate.meal.<slot>.end30`
   - 6 条 DAILY 远低于 iOS 64 上限
3. `src/store/useStore.ts`
   - 新增 `onboardingCompletedAt: number | null` 字段
   - `finishOnboarding` 设 `Date.now()`
   - schema v9 → **v10** migrate：已 onboarded 老用户回填 `0`（保旧行为），未 onboarded 保 `null`
4. `src/services/missedScan.ts` `detectMissedSlots`
   - 加 4th 参数 `onboardingCompletedAt: number | null`
   - 新规则：`windowEnd <= onboardingCompletedAt` 时跳过（onboarding 之前就过完的窗不算 missed）

### 验证

- `tsc --noEmit` clean
- JS 脚本 mirror 代码逻辑跑 10 case：
  - 07:59 不再显示 reminder（之前会，因为 8am - 90min = 6:30 起算）
  - 08:00 / 09:00 / 09:29 reminder 正确 ON；09:30 后 OFF
  - onboarding=14:00 + now=15:00 → 不 mark；+now=20:01 → mark dinner
  - 老用户 onboardingCompletedAt=0 + now=20:00 → mark 早午餐（保旧行为）
- ⬜ Simulator / 真机端到端：xin 待验（见 README v1.0.1 段 4 个场景）

### 同步文档

- `README.md` 加版本日志 + issue 进度表（#1-#7）
- `docs/PRD.md` §11.F 窗口定义改 ±90 → +90；加 §11.F.4 onboarding 守卫小节
- `docs/architecture/database.md` schema 表补 v8/v9/v10 + State 字段加 `onboardingCompletedAt`
- `docs/product/changelog.md` 加 v1.0.1 段
