...
- 用户点通知 → expo-router 路由到 `/(main)/photo?slot=...`
- settings 开发者面板有"立即触发"3 个按钮（早午晚），5 秒后弹一条测试推送

**xin 真机上要验证的**（我没法验，simulator DAILY trigger 偶尔跳过 + 锁屏行为只在真机准）：
- [ ] 首次启动是否弹 iOS 系统权限对话框（"mealmate 想给你发送通知"），点允许后无报错
- [ ] settings 开发者面板按"早餐/午餐/晚餐"，把 app 切到后台或锁屏，5 秒后是否真的弹横幅 + 有声音
- [ ] 点通知是否跳到 photo 屏，且 slot 参数对（早餐点了应该到早餐拍照流程）
- [ ] 把三餐时间改成 1 分钟后（settings 时间选择器），等到点是否真触发
- [ ] 安装新 native package 后必须重 pod install + run:ios：
  ...
  ```
- [ ] 真机装 dev build：`npx expo run:ios --device`（要插 USB + 信任开发者证书）

**已知 simulator 限制**（不算 bug）：
- DAILY trigger 在 simulator 系统时间被快进/休眠时偶尔跳过
- 锁屏样式、振动、声音只在真机完整呈现
