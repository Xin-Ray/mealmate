...
体重秤照片 → kg 数字。EasyOCR (CRNN, GPU) 推理，替换前端原来直连
Gemini Vision 的路径（issue：TestFlight 上 Google API 不可达）。

**Request**: `multipart/form-data`
- `image`: 图片文件，秤面尽量正面、不糊就行
...
数字提取规则：
- 正则匹配 `\d{1,3}(\.\d{1,2})?`（容忍纯数字 + 最多 2 位小数）
- 必须落在 20–250 kg 范围内（防误把"5" "999" 之类当体重）
- 多个候选时取 confidence 最高的
- 模型偶尔把整数 "605" 当 "60.5" 漏小数点 → 不主动修正，UI 显示原数让用户校对

客户端策略：`kg=null` 时回退到手填 input，`raw_text` 仅供调试 / 日志，不展示给用户。
